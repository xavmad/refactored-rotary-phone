...
setTimeout(() => {
  ...
  const SAFE_LEFT   = window.innerWidth * 0.05;
  const SAFE_RIGHT  = window.innerWidth * 0.52;
  const SAFE_TOP    = window.innerHeight * 0.07;
  const SAFE_BOTTOM = window.innerHeight * 0.93;
  ...
  groupImages.forEach((img, i) => {
    ...
    const radius = Math.sqrt(i) * SPIRAL_STEP;
    ...
    x = Math.max(SAFE_LEFT + halfW,
        Math.min(SAFE_RIGHT - halfW, x));

    y = Math.max(SAFE_TOP + halfH,
        Math.min(SAFE_BOTTOM - halfH, y));
