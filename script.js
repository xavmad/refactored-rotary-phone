...
setTimeout(() => {
  ...
  const IMAGE_HEIGHT = 220; // approximate visible height

  const halfW = IMAGE_WIDTH / 2;
  const halfH = IMAGE_HEIGHT / 2;

  const area = {
    x: window.innerWidth * 0.05 + halfW,
    y: window.innerHeight * 0.08 + halfH,
    w: window.innerWidth * 0.48 - halfW * 2,
    h: window.innerHeight * 0.84 - halfH * 2
  };

  const IMAGE_RADIUS = 190;
  const MAX_TRIES = 140;
  ...
  function overlaps(x, y) {
    for (const p of placed) {
      ...
      if (dist < IMAGE_RADIUS) return true;
    }
    return false;
  }
