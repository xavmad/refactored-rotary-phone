...
let camFrom = { scale: 1, x: 0, y: 0 };
let camTo   = { scale: 1, x: 0, y: 0 };
...
viewport.addEventListener('wheel', e => {
  ...
  if (camAnimating) return; // 🔒 prevent conflict
  e.preventDefault();
  ...
  targetScale = Math.min(Math.max(targetScale, MIN_SCALE), MAX_SCALE);

  originX = mouseX - worldX * targetScale;
  originY = mouseY - worldY * targetScale;
});
...
function smoothZoom() {
  if (camAnimating) {
    ...
    const t = Math.min((now - camStartTime) / camDuration, 1);
    const e = easeInOut(t);

    scale   = camFrom.scale + (camTo.scale - camFrom.scale) * e;
    originX = camFrom.x     + (camTo.x     - camFrom.x)     * e;
    originY = camFrom.y     + (camTo.y     - camFrom.y)     * e;

    if (t === 1) {
      camAnimating = false;
      targetScale = scale; // 🔑 hand control back to zoom system
    }
  } else {
    // normal zoom easing
...
function animateCamera(toScale, toX, toY) {
  camAnimating = true;
...
// Animate to reading view
animateCamera(1, 0, 0);


// Reset camera for group view
