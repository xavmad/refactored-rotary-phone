...
function floatImages() {
  if (!activeProject) {
    images.forEach(img => {
      ...
      // ✅ safety init (prevents jitter on load)
      ...
      // bounce bounds
      const m = 80;
      if (img._tx < m || img._tx > window.innerWidth - m) img._vx *= -1;
      if (img._ty < m || img._ty > window.innerHeight - m) img._vy *= -1;

      // ease actual position toward target
      x += (img._tx - x) * 0.04;
      y += (img._ty - y) * 0.04;
