...
/* =====================================================
   FLOATING MOTION (VECTOR — NO JITTER)
===================================================== */
...
function activateGroup(project) {
  ...
  setTimeout(() => {
   ...
   const imageHeight = 220;     // approx visible height
const spacing = 60;         // vertical breathing room
const totalHeight =
  groupImages.length * (imageHeight + spacing);

const availableHeight = bottom - top;

// scale spacing down only if too many images
const compression =
...
groupImages.forEach((img, i) => {
  ...
  const baseY =
    top +
    (i * (imageHeight + spacing)) * compression;

  const x =
    left +
    (Math.random() - 0.5) * 80; // subtle horizontal drift

  const y =
    baseY +
    Math.random() * 20 - 10; // organic feel
...
/* =====================================================
   DRAGGABLE PROJECT DESCRIPTION — FIXED
===================================================== */
...
descriptions.addEventListener("mousedown", e => {
  ...
  if (e.target.tagName === "P" || e.target.tagName === "STRONG") return;
  ...
  // 🔥 break centering rules
  descriptions.style.right = "auto";
  descriptions.style.transform = "none";
  descriptions.style.left = `${descLeft}px`;
  descriptions.style.top = `${descTop}px`;

  descriptions.style.transition = "none";
...
window.addEventListener("mousemove", e => {
  ...
  descriptions.style.left = `${descLeft + dx}px`;
  descriptions.style.top  = `${descTop + dy}px`;
});
...
window.addEventListener("mouseup", () => {
  ...
  descriptions.style.transition =
    "opacity 0.45s ease, transform 0.65s cubic-bezier(.19,1,.22,1)";
});
