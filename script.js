...
function cameraLoop() {
  ...
  if (Math.abs(zoomVelocity) > 0.000001) {
    ...
    // very slow decay → continuous feel
    zoomVelocity *= 0.96;
  }

  // soft cutoff (no snapping)
  if (Math.abs(zoomVelocity) < 0.0000005) {
    zoomVelocity = 0;
  }

  // damp pan during zoom
...
viewport.addEventListener('mousedown', e => {
  ...
  // 🔑 cancel zoom momentum immediately
  zoomVelocity = 0;

  isPanning = true;

  // 🔑 CRITICAL: sync pan start to CURRENT camera, not target
